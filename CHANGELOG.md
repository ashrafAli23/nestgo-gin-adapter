...
- `SendStream` (including SSE) no longer deadlocks clients that wait for headers before producing data (every real SSE/`EventSource` client): status and headers are now flushed to the wire at stream start, before the first read; streamed responses are therefore always chunked. The recorder and the defensive no-recorder path share one flush-before-first-read, flush-per-chunk `streamWriter` loop
- `ResponseBody()` returns a copy of the buffered body instead of the live pooled recorder buffer, so anything that keeps the slice (e.g. the Idempotency middleware's cache) can no longer read another request's bytes (the core contract already promised a copy)
